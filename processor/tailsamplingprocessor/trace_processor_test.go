...
func generateIdsAndBatches(numIds int) ([]pdata.TraceID, []pdata.Traces) {
	return generateTraceBatches(numIds)
}
...
func generateLogBatches(numIds int) ([]pdata.TraceID, []pdata.Logs) {
	...
	for i := 0; i < numIds; i++ {
		...
		for j := 0; j <= i; j++ {
			ld := simpleLogs()
			span := ld.ResourceLogs().At(0).InstrumentationLibraryLogs().At(0).Logs().At(0)
			span.SetTraceID(traceIds[i])
...
func (m *mockPolicyEvaluator) OnLateArrivingLogs(earlyDecision sampling.Decision, logs []*pdata.LogRecord) error {
	m.LateArrivingLogsCount++
	return m.NextError
}

func (m *mockPolicyEvaluator) EvaluateTrace(pdata.TraceID, *sampling.TraceData) (sampling.Decision, error) {
	m.EvaluationCount++
	return m.NextDecision, m.NextError
}

func (m *mockPolicyEvaluator) EvaluateLog(traceID pdata.TraceID, log *sampling.LogData) (sampling.Decision, error) {
	m.EvaluationCount++
	return m.NextDecision, m.NextError
}
